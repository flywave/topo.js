...
- `test/mirror.test.ts` — `mirror` 带 `ofFeature` 时**必须镜像该特征的工具体**。此前发射器完全忽略 `ofFeature` 一律镜像整个 body, 实测把 120×80×10 的板镜像成 **20 厚、体积翻倍**且无人察觉 (剪影沿草图法线取, 对厚度天然失明; 单视图也没有第二个视图可对账)。另钉死两条实测限制: 对已是镜像/union 结果的 Workplane 再调 `mirror` 会崩 "null function or function signature mismatch"; 而把两次反射合成为 180° 旋转虽数学等价, 实测形状错 (同一块板回来是 124.8 x 18.7), 故改为明确拒绝而非近似
