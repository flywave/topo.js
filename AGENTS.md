...
- **阈值也不是这张图的杠杆 —— 四种读法逐一量过, 全部失败** (2026-09). 那张图 (彩色卡通插图, 1080x1307): 亮度直方图 **84.8% 落在 240-255** (纸), 其余稀薄地摊在更暗的格子里, 没有双峰; Otsu 取 **161**。逐一试:
  - **亮度阈值扫 64→254**: 最大封闭区域占比 2.3%(232) → 3.7%(236) → **28.7%(240)** → 36.7%(244) → 63.6%(252)。240 处有相位跳变 (轮廓终于闭合成区域), 但**之后单调上升不收敛**, 与"局部吸附"同一个陷阱: 任何"最大化它"的规则都会选到极端。且渲染出来看, 240 处那个 858x1127px 的区域是**纸** (跨越整张图、一直伸到图底), 不是角色
  - **ink 模式 (最大的连通墨迹块)**: 240→252 全是 **bbox 覆盖整张图 (1080x125x)**, 面积 14.7%→24.6%, 渲染出来是**标注线连成的一张网** (边框 + 尺寸线 + 轮廓彼此相接), 不是角色; 254 处墨迹暴涨到 73% (全部并成一块)
  - **"彩色即图层" 无论亮度**: 彩色像素仅 3.78%, 最大的彩色连通块是 **图底一条 1080x44 的彩色横幅** (0.32%)。角色的填色**色度太低** (max-min ≤ 40) 被算作中性, 所以它既不在暗墨里也不在彩色层里
  - 结论: **这张图的零件对纸的对比度 (~1~5% 亮度、低色度) 比标注还小**, 于是任何能抓住零件的阈值都会抓进多得多得多的标注, 出来的结构是标注的结构。**不是参数没调对, 是模型不适用** (它是插图, 不是工程图)。真要做, 需要的是让视觉层给出零件所在区域 (ROI), 或做真正的色度分割 —— 都需要各自的证据, 不该顺手改
