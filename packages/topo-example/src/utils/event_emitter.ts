interface Callbacks {
  [namespace: string]: Record<string, Function[]>;
}
...
export default class EventEmitter {
  ...
  trigger(_name: string, _args: any[] = []): any {
    ...
    let name_arr = this.resolveNames(_name);

    // Resolve name
    let name = this.resolveName(name_arr[0]);
  ...
  private resolveName(name: string): EventName {
    const newName: EventName = { original: '', value: '', namespace: 'base' };
