...
// Define interfaces for the OpenCascade types we're using
...
const openCascadeHelper = {
  openCascade: null as OpenCascade | null,

  setOpenCascade(openCascade: OpenCascade) {
    this.openCascade = openCascade;
  },

  tessellate(shape: any): FaceData[] {
    if (!this.openCascade) {
      throw new Error('OpenCascade not initialized. Call setOpenCascade first.');
    }
  ...
  joinPrimitives(facelist: FaceData[]): [number[], number[], number[]] {
    let obP = 0;
    let obN = 0;
    let obTR = 0;
    let advance = 0;
  ...
  objGetTriangle(trianglenum: number, locTriIndices: number[]): [number[], number[], number[]] {
  ...
  generateGeometry(
    ...
    for (let i = 0; i < tot_triangle_count; i++) {
      const [vertices_idx] = this.objGetTriangle(i, locTriIndices);
      // first vertex
      v(
    ...
    for (let i = 0; i < tot_triangle_count; i++) {
      const [, normals_idx] = this.objGetTriangle(i, locTriIndices);
      f3(
        0 + i * 3,
        1 + i * 3,
        2 + i * 3,
